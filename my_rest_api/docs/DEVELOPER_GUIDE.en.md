...
6. Optional role validation is executed against the profile-driven authorization configuration.
7. The endpoint delegates to `CodexExecutionService`.
8. The service starts the Codex runtime, executes the task, and translates runtime errors into domain errors.
9. Global exception handlers generate a structured error response if needed.
10. The response provides the result, metadata, and headers like `X-Request-ID`.
