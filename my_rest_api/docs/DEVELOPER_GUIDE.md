...
6. Optional wird die Rollenprüfung gegen die profilgesteuerte Autorisierungskonfiguration ausgeführt.
7. Der Endpoint delegiert an `CodexExecutionService`.
8. Der Service startet die Codex-Laufzeit, führt den Task aus und übersetzt Laufzeitfehler in Domänenfehler.
9. Globale Exception-Handler erzeugen bei Bedarf ein strukturiertes Error-Response.
10. Die Response liefert Ergebnis, Metadaten und Header wie `X-Request-ID`.
